...
export default function Territorial() {
  ...
  // ── Confirmed indicados (territorial) ──
  ...
  const origemData = useMemo(() => {
    ...
    const labels: Record<string, string> = {
      formulario: "Formulário Público", manual: "Cadastro Manual",
      rede_social: "Rede Social", evento: "Evento", importacao: "Importação",
    };
    ...
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      <div>
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight flex items-center gap-2">
          ...
          Territorial & Recrutamento
        </h1>
        <p className="text-sm text-muted-foreground">
          Mapa de influência geográfica e painel de crescimento da base
        </p>
      </div>
      ...
      <Tabs defaultValue="mapa" className="w-full">
        <TabsList className="grid w-full grid-cols-2 max-w-sm">
          ...
          <TabsTrigger value="recrutamento" className="gap-1.5">
            <UserPlus className="w-4 h-4" /> Recrutamento
          </TabsTrigger>
        </TabsList>

        {/* ── Tab: Mapa Territorial ── */}
        <TabsContent value="mapa" className="space-y-4 sm:space-y-6 mt-4">
          {/* Stats */}
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
            <Card><CardContent className="pt-4 pb-3 px-4"><p className="text-xs text-muted-foreground">Total</p><p className="text-2xl font-bold">{supporters?.length || 0}</p></CardContent></Card>
            <Card><CardContent className="pt-4 pb-3 px-4"><p className="text-xs text-muted-foreground">Com localização</p><p className="text-2xl font-bold text-primary">{totalWithLocation}</p></CardContent></Card>
            <Card><CardContent className="pt-4 pb-3 px-4"><p className="text-xs text-muted-foreground">Sem localização</p><p className="text-2xl font-bold text-muted-foreground">{totalWithout}</p></CardContent></Card>
            <Card><CardContent className="pt-4 pb-3 px-4"><p className="text-xs text-muted-foreground">Regiões</p><p className="text-2xl font-bold">{groups.length}</p></CardContent></Card>
          ...
          {chartData.length > 0 && (
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2"><MapPin className="w-4 h-4 text-primary" />Distribuição por Região</CardTitle>
                <CardDescription className="text-xs">Top {chartData.length} regiões. Verde = zona quente, Amarelo = morna, Vermelho = fria.</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card className="border-destructive/30 bg-destructive/5">
              <CardContent className="pt-4 pb-3 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{groups.filter(g => g.count / maxCount < 0.4).length} regiões com poucos apoiadores — oportunidades de expansão</p>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          {/* Territory cards */}
          {filtered.length === 0 ? (
            <Card><CardContent className="py-12 text-center text-muted-foreground"><MapPin className="w-10 h-10 mx-auto mb-3 opacity-30" /><p className="font-medium">Nenhum dado territorial disponível</p><p className="text-xs mt-1">Os apoiadores precisam informar cidade/bairro no cadastro</p></CardContent></Card>
          ) : (
        ...
        {/* ── Tab: Recrutamento ── */}
        <TabsContent value="recrutamento" className="space-y-4 sm:space-y-6 mt-4">
          {pessoasLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                <MetricCard icon={Users} label="Total de cadastros" value={recruitMetrics.total} />
                <MetricCard icon={UserPlus} label="Hoje" value={recruitMetrics.today} accent />
                <MetricCard icon={CalendarDays} label="Últimos 7 dias" value={recruitMetrics.week} />
                <MetricCard icon={TrendingUp} label="Últimos 30 dias" value={recruitMetrics.month} />
              </div>

              {/* Growth chart */}
              <Card>
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2"><BarChart3 className="w-5 h-5 text-primary" /><CardTitle className="text-base">Cadastros por Dia — Últimos 30 dias</CardTitle></div>
                </CardHeader>
                <CardContent>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2"><BarChart3 className="w-4 h-4 text-primary" /><CardTitle className="text-sm">Origem dos Cadastros</CardTitle></div>
                </CardHeader>
                <CardContent>
                  {origemData.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {origemData.map((o) => (
                        <DistributionRow key={o.name} label={o.name} count={o.count} total={recruitMetrics.total} />
                      ))}
                    </div>
                  )}
                </CardContent>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2"><Clock className="w-5 h-5 text-primary" /><CardTitle className="text-base">Últimos Cadastros</CardTitle></div>
                </CardHeader>
                <CardContent>
